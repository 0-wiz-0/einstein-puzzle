...
Puzzle::Puzzle(IconSet &is, SolvedPuzzle &s, Possibilities *p)
    : possib(p), iconSet(is), valid(true), win(false), solved(s), hCol(-1),
      hRow(-1), subHNo(-1), winCommand(nullptr), failCommand(nullptr) {
    reset();
}
...
void Puzzle::drawCell(int col, int row) {
    ...
    if (possib->isDefined(col, row)) {
        ...
        if (element > 0) {
            screen.drawScaled(
                posX, posY,
                iconSet.getLargeIcon(row, element,
                                     (hCol == col) && (hRow == row)));
        }
    }
    else {
        ...
        for (int i = 0; i < 6; i++) {
            if (possib->isPossible(col, row, i + 1)) {
                SDL_Surface *origIcon = iconSet.getLargeIcon(
                    row, i + 1,
...
bool Puzzle::getCellNo(int x, int y, int &col, int &row, int &subNo) {
    ...
    y = y - row * (FIELD_TILE_HEIGHT + FIELD_GAP_Y) - FIELD_TILE_HEIGHT / 6;
