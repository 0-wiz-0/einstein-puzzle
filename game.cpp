...
GameBackground::GameBackground() {
    ...
    add(new ManagedLabel(L"nova.ttf", 28, 20, 20, 255, 255, 0,
                         msg(L"einsteinPuzzle"), true));
}
...
void Watch::draw() {
    ...
    font->getSize(s, w, h);
    SDL_Rect rect
        = {scaleUp(x - 2), scaleUp(y - 2), w + scaleUp(4), h + scaleUp(4)};
...
class WinCommand : public Command {
  ...
  public:
    ...
    void doAction() override {
        ...
        showMessageWindow(gameArea, L"marble1.bmp", 500, 70, &font, 255, 0, 0,
                          msg(L"won"));
        gameArea->draw();
        TopScores scores;
...
class OkDlgCommand : public Command {
  ...
  public:
    OkDlgCommand(Area *a, bool &r) : res(r) { area = a; }
...
class CheatCommand : public Command {
  ...
  public:
    ...
    void doAction() override {
        ...
        showMessageWindow(gameArea, L"darkpattern.bmp", 500, 100, &font, 255,
                          255, 255, msg(L"iddqd"));
        gameArea->draw();
    }
};
...
class GameOptionsCommand : public Command {
  ...
  public:
    explicit GameOptionsCommand(Area *a) { gameArea = a; }
...
#define BUTTON(x, y, text, cmd)                                          \
    area.add(new Button(x, y, 94, 30, &btnFont, 255, 255, 0, L"btn.bmp", \
                        msg(text), false, cmd));
