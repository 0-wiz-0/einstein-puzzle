...
class CancelCommand : public ExitCommand {
  public:
    explicit CancelCommand(Area &a) : ExitCommand(a) {}
...
#define CHECKBOX(y, var) \
    area.add(            \
        new Checkbox(265, y, 20, 20, &font, 255, 255, 255, L"blue.bmp", var));
...
class OptionsButton : public Button {
  public:
    OptionsButton(int x, int y, int w, int h, Font *font, int r, int g, int b,
                  const std::wstring &bg, const std::wstring &text,
                  Command *cmd)
...
void showOptionsWindow(Area *parentArea) {
    ...
    area.add(new OptionsButton(405, 390, 85, 25, &font, 255, 255, 0,
                               L"blue.bmp", msg(L"cancel"), &exitCmd));
