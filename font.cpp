    ...
    if ((!str) || (sizeof(wchar_t) == sizeof(Uint16))) {
        ...
        if (!convBuf) {
            ...
            if (!buf) {
                free(convBuf);
                throw Exception(
                    L"Buffer realloc failed during string conversion");
            }
            convBuf = buf;
            bufSize = sz;
        }
...
Font::Font(const std::wstring &name, int ptsize) : name(name), uSize(ptsize) {
...
void Font::draw(SDL_Surface *s, int x, int y, int r, int g, int b, bool shadow,
