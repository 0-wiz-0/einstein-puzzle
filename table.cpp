    ...
    case Lexeme::Symbol:
        if (L"{" == lexeme.getContent()) {
            return new TableValue(
                new Table(lexal, lexeme.getLine(), lexeme.getPos()));
        }
    default:
    ...
    while (true) {
        ...
        if (Lexeme::Eof == type) {
            if (!needBracket) {
                ...
                throw Exception(L"Table started at "
                                + Lexal::posToStr(startLine, startPos)
                                + L" is never finished");
            }
        }
            ...
            if (Lexeme::Symbol == nextLex.getType()) {
                ...
                throw Exception(L"Unexpected token at "
                                + Lexal::posToStr(startLine, startPos));
            }
        }
        else {
...
int Table::getInt(const std::wstring &key, int dflt) const {
    ValuesMap::const_iterator i = fields.find(key);
    return (i != fields.end()) ? ((*i).second ? (*i).second->asInt() : dflt)
                               : dflt;
}
...
void Table::save(const std::wstring &fileName) const {
    std::ofstream stream(toMbcs(fileName).c_str(),
