...
class TextCommand : public MsgCommand {
  ...
  public:
    ...
    std::wstring toString() override { return L"text: '" + text + L"'"; }
...
class IntArgCommand : public ArgCommand {
  public:
    ...
    std::wstring toString() override { return L"int_arg " + ::toString(argNo); }
...
class StrArgCommand : public ArgCommand {
  public:
    ...
    std::wstring toString() override { return L"str_arg " + ::toString(argNo); }
    ...
    while (i < len) {
        ...
        if ((L'%' == ch) && (i != len - 1)) {
            ...
            if (L'%' != ch) {
                ...
                case L'd':
                case L'i':
                    cmd = new IntArgCommand(argNo);
                    break;
                case L's':
                    cmd = new StrArgCommand(argNo);
                    break;
                case L'f':
                    cmd = new FloatArgCommand(argNo);
                    break;
                case L'e':
                    cmd = new DoubleArgCommand(argNo);
                    break;
                default:
                    throw Exception(std::wstring(L"Format string '%") + ch
                                    + std::wstring(L"' is not supported"));
                }
