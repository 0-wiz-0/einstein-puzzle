...
HorizontalRule::HorizontalRule(std::istream &stream,
                               const std::wstring &ruleType)
...
NearRule::NearRule(SolvedPuzzle puzzle) {
    ...
    else if (col1 == PUZZLE_SIZE - 1) {
        col2 = PUZZLE_SIZE - 2;
    }
    else if (rndGen.genInt(2)) {
        col2 = col1 + 1;
    }
    else {
        col2 = col1 - 1;
...
NearRule::NearRule(std::istream &stream) : HorizontalRule(stream, L"near") {
}
...
std::wstring NearRule::getAsText() {
    return getThingName(row1, thing1) + L" is near to "
...
std::wstring DirectionRule::getAsText() {
    return getThingName(row1, thing1) + L" is from the left of "
           + getThingName(row2, thing2);
}
...
UnderRule::UnderRule(std::istream &stream) : DrawableRule(stream, L"under") {
}
        ...
        for (int i = 0; i < PUZZLE_SIZE; i++) {
            ...
            if (pos.isPossible(i, row2, thing2)) {
                if (i < 2) {
                    ...
                    leftPossible
                        = (pos.isPossible(i - 1, centerRow, centerThing)
...
std::wstring BetweenRule::getAsText() {
    return getThingName(centerRow, centerThing) + L" is between "
           + getThingName(row1, thing1) + L" and " + getThingName(row2, thing2);
}
...
Rule *genRule(SolvedPuzzle &puzzle) {
    ...
    case 2:
    case 3:
        return new NearRule(puzzle);
    case 4:
        return new OpenRule(puzzle);
    case 5:
    case 6:
        return new UnderRule(puzzle);
    case 7:
    case 8:
    case 9:
    case 10:
        return new DirectionRule(puzzle);
    case 11:
    case 12:
    case 13:
        return new BetweenRule(puzzle);
    default:
