...
SavedGame::SavedGame(const std::wstring &s) : fileName(s) {
    ...
    try {
        std::ifstream stream(toMbcs(fileName).c_str(),
...
class SaveCommand : public Command {
  ...
  public:
    SaveCommand(SavedGame &sg, Font *f, Area *area, bool *s,
                const std::wstring &dflt, Game *g)
        : savedGame(sg), defaultName(dflt) {
        parentArea = area;
        saved = s;
        font = f;
        game = g;
  ...
  public:
    void doAction() override {
        ...
        area.add(new InputField(340, 300, 280, 26, L"blue.bmp", name, 20, 255,
                                255, 0, font));
        ...
        if (*saved) {
            *saved = false;
            try {
                std::ofstream stream(toMbcs(savedGame.getFileName()).c_str(),
                                     std::ofstream::out
                                         | std::ofstream::binary);
                if (stream.fail()) {
                ...
                *saved = true;
                ...
                showMessageWindow(&area, L"redpattern.bmp", 300, 80, font, 255,
                                  255, 255, msg(L"saveError"));
            }
...
bool saveGame(Area *parentArea, Game *game) {
    ...
    for (int i = 0; i < MAX_SLOTS; i++) {
        ...
        list.push_back(sg);
        commands[i] = new SaveCommand(*(--(list.end())), &font, &area, &saved,
                                      L"game " + toString(i + 1), game);
    }
...
class LoadCommand : public Command {
  ...
  public:
    LoadCommand(SavedGame &sg, Font *f, Area *area, Game **g) : savedGame(sg) {
        parentArea = area;
        font = f;
        game = g;
    }
    ...
    void doAction() override {
        try {
            ...
            *game = g;
            ...
            showMessageWindow(parentArea, L"redpattern.bmp", 300, 80, font, 255,
                              255, 255, L"Error loadng game");
        }
        parentArea->finishEventLoop();
    }
};
...
Game *loadGame(Area *parentArea) {
    ...
    for (int i = 0; i < MAX_SLOTS; i++) {
        ...
        if (sg.isExists()) {
            commands[i]
                = new LoadCommand(*(--(list.end())), &font, &area, &newGame);
        }
        else {
            commands[i] = nullptr;
        }
